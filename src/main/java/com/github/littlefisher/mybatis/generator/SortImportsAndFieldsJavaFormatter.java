...
public class SortImportsAndFieldsJavaFormatter implements JavaFormatter {
    ...
    private String getInterfaceFormattedContent(CompilationUnit compilationUnit) {
        ...
        interfaceClazz.getFileCommentLines()
            .forEach(commentLine -> {
                sb.append(commentLine);
                OutputUtilities.newLine(sb);
            });

        formatPackage(sb, interfaceClazz.getType()
            .getPackageName());

        formatStaticImports(sb, interfaceClazz.getStaticImports());

        if (interfaceClazz.getStaticImports()
            .size() > 0) {
            OutputUtilities.newLine(sb);
        }
        ...
        sb.append(interfaceClazz.getVisibility()
            .getValue());
        ...
        sb.append("interface ");
        sb.append(interfaceClazz.getType()
            .getShortName());

        if (interfaceClazz.getSuperInterfaceTypes()
            .size() > 0) {
            sb.append(" extends ");
        ...
        Iterator<Method> mtdIter = interfaceClazz.getMethods()
            .iterator();
    ...
    private String getClassFormattedContent(CompilationUnit compilationUnit) {
        ...
        topLevelClass.getFileCommentLines()
            .forEach(fileCommentLine -> {
                sb.append(fileCommentLine);
                OutputUtilities.newLine(sb);
            });

        formatPackage(sb, topLevelClass.getType()
            .getPackageName());

        formatStaticImports(sb, topLevelClass.getStaticImports());

        if (topLevelClass.getStaticImports()
            .size() > 0) {
            OutputUtilities.newLine(sb);
        }
